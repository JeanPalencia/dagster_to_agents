...
Versión 3 (Dagster) - Usa credenciales AWS SSM en lugar de archivos .env.

Copia adaptada de update_spot_status_history_v3.py para validación contra
el pipeline Dagster. La lógica de procesamiento es idéntica al original.

Uso:
    python update_spot_status_history_v3_dagster.py
"""
...
OUTPUT_PARQUET_V3 = os.path.join(OUT_DIR, "final_spot_status_full_df_v3_dagster.parquet")
OUTPUT_CSV_V3 = os.path.join(OUT_DIR, "final_spot_status_full_df_v3_dagster.csv")
...
# =============================================================================
# FUNCIONES DE PROCESAMIENTO (idénticas al original)
# =============================================================================
...
def get_mysql_connection():
    username = _get_ssm_param("/dagster/DB_ANALYSIS_USERNAME")
    password = _get_ssm_param("/dagster/DB_ANALYSIS_PASSWORD")
...
def update_incremental():
    """Ejecuta el pipeline incremental y retorna el DataFrame."""
    print("="*60)
    print("ACTUALIZACIÓN INCREMENTAL DE SPOT STATUS HISTORY (V3-DAGSTER)")
    print("="*60)
    ...
    df_rebuilt.to_csv(OUTPUT_CSV_V3, index=False)
    print(f"    -> {OUTPUT_CSV_V3} ({len(df_rebuilt):,} registros)")
