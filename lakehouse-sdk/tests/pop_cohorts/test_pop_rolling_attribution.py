#!/usr/bin/env python3
"""
Test: Pop-Rolling with Attribution Segmenters.

Runs the original Pop-Rolling query and the modified version (with channel,
...
Usage:
    cd dagster-pipeline
    uv run python ../lakehouse-sdk/tests/pop_cohorts/test_pop_rolling_attribution.py
"""
...
METRIC_COLS = [
    ...
    "projects_current",
    "projects_prev",
    "request_visit_current",
    "request_visit_prev",
    "completed_visit_current",
    "completed_visit_prev",
    "lois_current",
    "lois_prev",
    "won_current",
    "won_prev",
...
def main() -> None:
    ...
    _separator("1. Running ORIGINAL Pop-Rolling query")
    original_sql = (SQL_DIR / "Pop-Rolling.sql").read_text()
    t0 = time.time()
    ...
    _separator("2. Running MODIFIED Pop-Rolling query (with attribution)")
    modified_sql = (SQL_DIR / "Pop-Rolling-with-attribution.sql").read_text()
    t0 = time.time()
