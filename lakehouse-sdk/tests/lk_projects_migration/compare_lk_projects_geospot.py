#!/usr/bin/env python3
"""
Comparacion directa en Geospot: lk_projects (governance) vs lk_projects_v2 (legacy).

Compara fila a fila y columna a columna usando project_id como llave.
...
DEFAULT_GOV_TABLE = "lk_projects"
DEFAULT_LEGACY_TABLE = "lk_projects_v2"
DEFAULT_SAMPLE = 20
PK = "project_id"
...
def _to_comparable(df: pl.DataFrame) -> pl.DataFrame:
    """Cast every column to Utf8 for consistent value comparison."""
    exprs = []
    for col_name in df.columns:
        dtype = df[col_name].dtype
        if dtype == pl.List(pl.Utf8) or str(dtype).startswith("List"):
            exprs.append(
                pl.col(col_name)
                .map_elements(lambda v: str(v) if v is not None else None, return_dtype=pl.Utf8)
                .alias(col_name)
            )
        else:
            exprs.append(pl.col(col_name).cast(pl.Utf8, strict=False).alias(col_name))
    return df.select(exprs)


def main() -> None:
    parser = argparse.ArgumentParser(description="Compara lk_projects vs lk_projects_v2 en Geospot")
    ...
    if args.report_md:
        ...
        report_path = (
            Path(__file__).resolve().parent / "reports" / f"lk_projects_comparison_{ts}.md"
        )
    ...
    # --- 3. Load data ---
    # Detect jsonb columns to cast them to text in SQL (avoids pyarrow conversion errors)
    gov_type_map = {
    ...
    if type_mismatch_df.height:
        ...
        L.append("")
        L.append("_Nota: `text` vs `character varying` son equivalentes en PostgreSQL._")
    else:
