...
from dagster_pipeline.defs.lk_matches_visitors_to_leads.utils.database import get_engine, query_bigquery
...
from dagster_pipeline.defs.lk_matches_visitors_to_leads.utils.dataframe_utils import safe_concat_multiple


# Path to queries directory (relative to this file)
QUERIES_DIR = Path(__file__).parent / "queries"

# Origen de lead_events: True = lead_events_from_funnel.sql (eventos + atribución funnel), False = lead_events.sql (raw GA4)
USE_FUNNEL_FOR_LEAD_EVENTS = True
...
def get_data(kind: str) -> pd.DataFrame:
    ...
    match kind:
        case "clients":
            ...
            return clients
        case "lead_events":
            query_file = "lead_events_from_funnel.sql" if USE_FUNNEL_FOR_LEAD_EVENTS else "lead_events.sql"
            query_path = QUERIES_DIR / query_file
            with open(query_path, "r", encoding="utf-8") as f:
                query = f.read()
            return query_bigquery(query)
        case "lead_events_lk":
            query_path = QUERIES_DIR / "lead_events_from_funnel_lk.sql"
            with open(query_path, "r", encoding="utf-8") as f:
                query = f.read()
            return query_bigquery(query)
        case _:
