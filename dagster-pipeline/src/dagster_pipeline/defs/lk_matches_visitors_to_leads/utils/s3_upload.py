...
def save_to_local(
    ...
    table_basename: str = "temp_matches_users_to_leads",
) -> str:
    ...
    path = out / f"{table_basename}_{ts}.{ext}"
    df_export = df.copy()
    if "user_pseudo_id" in df_export.columns:
        df_export["user_pseudo_id"] = df_export["user_pseudo_id"].astype("string")
    if "vis_user_pseudo_id" in df_export.columns:
        df_export["vis_user_pseudo_id"] = df_export["vis_user_pseudo_id"].astype("string")
...
# LK_MAT table (única salida: LK_MAT_MATCHES_VISITORS_TO_LEADS, PK lead_id + lead_fecha_cohort_dt)
S3_KEY_MAT = "lk_mat_matches_visitors_to_leads/lk_mat_matches_visitors_to_leads.csv"
S3_BACKUP_PREFIX_MAT = "lk_mat_matches_visitors_to_leads/backups"
TABLE_NAME_MAT = "lk_mat_matches_visitors_to_leads"
...
def resolve_table_and_s3_prefix(table_name_prefix: str = "") -> tuple[str, str, str]:
    """Resuelve nombre de tabla MAT, S3 key y backup prefix aplicando opcionalmente un prefijo de prueba.

    Útil para no sobreescribir producción cuando table_name_prefix = 'test_' (p. ej. en esta branch).

    Args:
        table_name_prefix: Prefijo opcional (ej. 'test_'). Si vacío, se usan nombres de producción.

    Returns:
        (table_name, s3_key, backup_prefix) para lk_mat_matches_visitors_to_leads.
    """
    base_name = TABLE_NAME_MAT
    s3_key = S3_KEY_MAT
    backup_prefix = S3_BACKUP_PREFIX_MAT

    prefix = (table_name_prefix or "").strip()
    if not prefix:
        return base_name, s3_key, backup_prefix

    table_name = prefix + base_name
    s3_key_prefixed = f"{prefix}{base_name}/{base_name}.csv"
    backup_prefix_prefixed = f"{prefix}{base_name}/backups"
    return table_name, s3_key_prefixed, backup_prefix_prefixed
...
def upload_dataframe_to_s3(
    ...
    Args:
        ...
        backup_prefix: S3 prefix for backup (e.g. lk_mat_matches_visitors_to_leads/backups). If None, uses S3_BACKUP_PREFIX_MAT.
    ...
    if create_backup:
        ...
        prefix = backup_prefix if backup_prefix is not None else S3_BACKUP_PREFIX
        backup_key = f"{prefix}/{today}.csv"
        s3.put_object(
            Bucket=S3_BUCKET_NAME,
            Key=backup_key,
            Body=csv_content,
